...
class StatusToTwist(object):
    ...
    def __init__(self):
        ...
        self.transmission_type = rospy.get_param("~transmission", "auto") # getting trasnmission type from param
    ...
    def cb_status(self, msg):
        """
            ...
            publish: /cmd_vel
        """
        if self.cmd_publish and not self.last_led and not self.emergancy_status:            
            self.set_color([0,1,0])
        
        if self.cmd_publish and not self.last_led and self.emergancy_status:            
            self.set_color([1,1,0])
